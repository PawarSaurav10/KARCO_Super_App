...
const InProgressCard = ({ VideoName, remainingTime, VideoCategory, userLoginData, VideoId, videoPassword, Percentage, NoOfQuestionAnswered, ProgressNo, TotalQuestion }) => {
            ...
            <View style={{ backgroundColor: COLORS.primary, paddingHorizontal: 12, paddingVertical: 16, margin: 4, borderRadius: 24, position: "absolute", width: Dimensions.get("window").width - 40, height: 180, justifyContent: "space-between" }}>
                ...
                <View>
                    <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "center", marginBottom: 4 }}>
                        <Text style={{ color: COLORS.white2 }}>Done {NoOfQuestionAnswered}/{"20"}</Text>
                        <Text style={{ color: COLORS.white2 }}>{(Percentage).toFixed(0)}%</Text>
                    </View>
                    <View style={{ marginBottom: 10 }}>
                        <Progress.Bar progress={0.5} animationType="timing" width={200} height={4} color="yellow" borderWidth={1} unfilledColor="white" />
                    </View>
                </View>
